...
//dummy users
function addUser(username, health, money, weapon, item, magik, strength, weight, score, winner) {
                ...
                if(err) {
                    ...
                    con.query("SELECT item_id FROM items WHERE name = '" + item + "'", function(err, curitem) {
                        if(err) {
                            ...
                            con.query("INSERT INTO users VALUE(NULL, '" + username + "', " + health + ", " + money + ", " + strength + ", " + weight + ", " + score + ", "+winner+", " + curweapon[0].weapon_id + ", " + curitem[0].item_id + ", " + curmagik[0].magik_id + ")", function (err, result) {
                                if (err) {
                                    throw err;
                                } else {
...
//addUser("name", health, gold, "weapon", "item", "magik", strength, weight, score, winner);
addUser("Clementine", 0, 200, "Whip", "None", "Dark", 17, 1, 53, 0);
addUser("Trollface", 0, 15, "Fish", "None", "Light", 11, 1, 72,  0);
addUser("RockGoblin", 0, 72, "Stick", "Chain mail", "Nature", 13, 3, 24, 0);
addUser("Snoop Dog", 0, 420, "Fists", "Fluffy Cloud", "Air", 12, 2, 28, 0);
addUser("???", 50, 150, "Sword", "None", "Dark", 20, 2, 95, 1);